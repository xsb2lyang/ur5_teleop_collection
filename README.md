...
```
robot_dataset/
└── pick_up_the_red_block/  <-- 以您输入的语言指令命名的文件夹
    ├── session_20250810_151820/  <-- 第一个样本 (Trajectory 1)
    │   ├── language.txt
    │   ├── robot_action.npy
    │   ├── robot_state.npy
    │   ├── d435_color/
    ...
    │   └── zed_color/
    │
    └── session_20250810_151835/  <-- 第二个样本 (Trajectory 2)
        └── ...
```

## **📊 输出数据格式详解**

每个 `session` 文件夹都包含一个完整的、多模态的任务样本。
  ...
  * **图像文件夹**
      ...
      * **内容**: `d435_color/`, `d435_depth/`, `zed_color/` 文件夹内存放着对应数据流的 `.png` 图像文件。
      * **命名**: 文件名以Unix时间戳命名（如 `1722984590.1234.png`），可用于精确的时间对齐。
